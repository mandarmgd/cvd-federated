...
def loadData(dir):
    ...
    image_list = []
    label_list = []
    labels = {'Abnormal heartbeat' : 0, 'History of MI' : 1, 'Normal Person' : 2}
...
X_train1, y_train1 = loadData("ECG Dataset/TrainingSets/TrainingSet1")
X_train2, y_train2 = loadData("ECG Dataset/TrainingSets/TrainingSet2")
X_train3, y_train3 = loadData("ECG Dataset/TrainingSets/TrainingSet3")
...
min_len = min(averageResult1.shape[0], averageResult2.shape[0], averageResult3.shape[0])
averageResult1_trimmed = averageResult1[:min_len]
averageResult2_trimmed = averageResult2[:min_len]
averageResult3_trimmed = averageResult3[:min_len]
averageResult = (averageResult1_trimmed + averageResult2_trimmed + averageResult3_trimmed) / 3
label_list = np.concatenate((label_list1[:min_len], label_list2[:min_len], label_list3[:min_len]), axis=0)
extracted_Features = pd.concat([
    extracted_Features1.iloc[:min_len],
    extracted_Features2.iloc[:min_len],
    extracted_Features3.iloc[:min_len]
], ignore_index=True)
